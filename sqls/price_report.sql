CREATE OR REPLACE VIEW price_report AS 
 WITH page_pulls_with_recency AS (
         SELECT page_pulls.id,
            ...
            rank() OVER (PARTITION BY page_pulls.location ORDER BY page_pulls.created_at DESC) AS daterank
           FROM page_pulls
        ), floor_plans_ranked AS (
         SELECT floor_plans.trust,
            ...
            rank() OVER (PARTITION BY floor_plans.trust, floor_plans.location, floor_plans.name, floor_plans.beds, floor_plans.baths, floor_plans.sqft ORDER BY floor_plans.created_at) AS daterank
           FROM floor_plans
        ), joined AS (
         SELECT pp.trust,
            pp.location,
         ...
         SELECT joined.trust,
            ...
            joined.fetched_at,
            joined.current,
            rank() OVER (PARTITION BY joined.trust, joined.location, joined.unitname, joined.unitnum, joined.rent, joined.movein ORDER BY joined.fetched_at DESC, joined.sqft_per_dollar DESC, uuid_generate_v4()) AS drank
           FROM joined
        )
 SELECT ranked.trust,
    ranked.location,
